...
export class AddRentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.tenantForm = this.formBuilder.group({
      ...
      TenantEmail: new FormControl(null, [
        Validators.required,
        Validators.pattern(/^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$/),
      ]),
      RentedSpaceInSqmtr: new FormControl('', [
        Validators.required,
        Validators.max(this.PropertyData.LandSize),
        Validators.min(1),
      ]),
      RentedSpace: new FormControl('Whole', Validators.required),
      RentDuration: new FormControl(1, [
        Validators.required,
        Validators.min(1),
      ]),
      RentDueDay: new FormControl('1', [
        Validators.required,
        Validators.maxLength(255),
        Validators.min(1),
      ]),
      MonthlyORDailyRent: new FormControl('', [
        Validators.required,
        Validators.min(1),
      ]),
      AdvanceDeposite: new FormControl(null, [
        Validators.required,
        Validators.min(1),
      ]),
  ...
  prepareSave(): any {
    ...
    input.append(
      'FileName',
      this.tenantForm.get('FileName').value + '.' + this.fileExtension
    );
    ...
    input.append(
      'BankAccountName',
      this.tenantForm.get('BankAccountName').value
    );
    input.append('GSTNumber', this.tenantForm.get('GSTNumber').value);
    input.append(
      'BankAccountNumber',
      this.tenantForm.get('BankAccountNumber').value
    );
    ...
    input.append(
      ...
      this.tenantForm.get('TransactionSMSMobileNo').value
    );
    input.append(
      'Cheque_Favour_OF',
      this.tenantForm.get('Cheque_Favour_OF').value
    );
    ...
    input.append(
      'RentedSpaceInSqmtr',
      this.tenantForm.get('RentedSpaceInSqmtr').value
    );
    input.append(
      'AdvanceDeposite',
      this.tenantForm.get('AdvanceDeposite').value
    );
    input.append(
      'MonthlyORDailyRent',
      this.tenantForm.get('MonthlyORDailyRent').value
    );
    input.append(
      'ContractStartDate',
      this.tenantForm.get('ContractStartDate').value
    );
  ...
  setInitialValue() {
    this.service.viewTenant(this.route.snapshot.params.id).subscribe((Res) => {
      ...
      this.tenantForm
        .get('AdvanceDeposite')
        .setValue(Res.data[0].AdvanceDeposite);
      this.tenantForm
        .get('ContractStartDate')
        .setValue(moment(Res.data[0].ContractStartDate).format('YYYY-MM-DD'));
      this.tenantForm
        .get('ContractEndDate')
        .setValue(moment(Res.data[0].ContractEndDate).format('YYYY-MM-DD'));
  ...
  onSubmit() {
    ...
    if (this.tenantForm.valid && this.isEdit === false) {
      this.isLoading = true;
      this.service
        .addTenant(this.prepareSave(), this.propertyId)
        .subscribe((data) => {
          this.isLoading = false;
      ...
      this.isLoading = true;
      this.service
        .updateTenant(this.tenantForm.value, this.route.snapshot.params.id)
        .subscribe((data) => {
          this.isLoading = false;
          if (data.status === 200) {
            Swal.fire({
